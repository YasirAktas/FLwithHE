...
def normalize_scheme(value: object) -> str:
    ...
    if "+" in key:
        ...
        if parts:
            mapped_parts = [SCHEME_LABELS.get(p, p.upper()) for p in parts]
...
def generate_integer_stats_outputs(df: pd.DataFrame, output_dir: str):
    summary = mean_by_scheme(df, FULL_MODEL_SUMMARY_COLUMNS)
    payload_scaled, payload_unit = bytes_scale(summary["payload_nbytes"])
    summary = summary.copy()
    summary["payload_scaled"] = payload_scaled
    ...
    plot_bar(
        summary, "scheme_label", "he_total_time",
        "Integer Stats HE Total Time Comparison", "Scheme", "Average HE Total Time (s)",
        os.path.join(output_dir, "integer_he_total_time.png"),
    )
    plot_bar(
        summary, "scheme_label", "payload_scaled",
        "Integer Stats Payload Size Comparison", "Scheme", f"Average Payload Size ({payload_unit})",
        os.path.join(output_dir, "integer_payload_size.png"),
    )
    plot_bar(
        summary, "scheme_label", "ciphertext_count",
        "Integer Stats Ciphertext Count Comparison", "Scheme", "Average Ciphertext Count",
        os.path.join(output_dir, "integer_ciphertext_count.png"),
    )
    plot_line(
        mean_by_round(df, "accuracy"), "round", "accuracy",
        "Integer Stats Accuracy vs Round", "Round", "Accuracy",
        os.path.join(output_dir, "integer_accuracy_vs_round.png"),
    )
    plot_line(
        mean_by_round(df, "loss"), "round", "loss",
        "Integer Stats Loss vs Round", "Round", "Loss",
        os.path.join(output_dir, "integer_loss_vs_round.png"),
    )
    plot_bar(
        summary, "scheme_label", "mean_abs_error",
        "Integer Stats Aggregation Error Comparison", "Scheme", "Average Mean Absolute Error",
        os.path.join(output_dir, "integer_mean_abs_error.png"),
    )
    if {"integer_reference", "integer_decrypted"}.issubset(df.columns):
        plot_reference_vs_decrypted(
            df,
            "integer_reference",
            "integer_decrypted",
            "Integer Stats Reference vs Decrypted",
            os.path.join(output_dir, "integer_reference_vs_decrypted.png"),
        )
    save_summary_csv(df, ["scheme_label"], FULL_MODEL_SUMMARY_COLUMNS, os.path.join(output_dir, "summary_integer_stats.csv"))
...
def main():
    ...
    elif args.payload_mode == "integer_stats":
        generate_integer_stats_outputs(df, args.output_dir)
    else:
        raise ValueError("Plot generation is currently implemented for full_model, analytics, integer_stats, and sweep outputs.")
